...
def perturb(graph_list, p_del, p_add=None):
    ...
    for G_original in graph_list:
        ...
        if p_add is None:
            ...
            p_add_est = (
                np.sum(trials) / (num_nodes * (num_nodes - 1) / 2 - G.number_of_edges())
            )
        else:
            p_add_est = p_add
...
def draw_graph_list(
    G_list, row, col, fname='figures/test', layout='spring', is_single=False, k=1, node_size=55, alpha=1, width=1.3
):
    # # draw graph view
