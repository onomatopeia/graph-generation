...
from model import (
    binary_cross_entropy_weight, sample_sigmoid, sample_sigmoid_supervised,
    sample_sigmoid_supervised_simple,
)
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        for i in range(max_num_node):
            ...
            x_step = sample_sigmoid_supervised(
                y_pred_step, y[:, i:i + 1, :].cuda(), current=i, y_len=y_len, sample_time=sample_time
            )
...
def train_rnn_epoch(
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        loss_sum += current_loss * feature_dim
    return loss_sum / (batch_idx + 1)
